/*:
 ## Exercise - Constante ou Variable?
 
 Imaginez que vous créez une application de partage de photos simple. Vous souhaitez suivre les statistiques suivantes pour chaque publication:
 - numbersOfLikes: nombre de likes qu'une photo a reçus
 - numbersOfComments: nombre de commentaires laissés par les autres utilisateurs sur la photo.
 - year: année de création du poste.
 - month: le mois de création du poste représenté par un nombre compris entre 1 et 12
 - day: jour du mois où le message a été créé.
 
Pour chacune des mesures ci-dessus, déclarez une constante ou une variable et attribuez-lui une valeur correspondant à une publication hypothétique. Veillez à utiliser les conventions de dénomination appropriées.
 */
var numbersOfLikes = 100
var numbersOfComments = 50
let year = 2022
let month = 6
let day = 14

print("This post has \(numbersOfLikes) likes.\nThis post has \(numbersOfComments) comments.\nIt was posted on \(month)/\(day)/\(year).")



//: [Previous](@previous)  |  page 5 of 10  |  [Next: App Exercise - Fitness Tracker: Constant or Variable?](@next)
